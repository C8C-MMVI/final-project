...
/* ─── Section → activeSection key map (unchanged from original) ─── */
...
const NAV = {
  admin: [
    ...
    { section: 'Platform' },
    { label: 'Repairs',         page: 'repairs',        icon: <IconTool />     },
    ...
    { section: 'Account' },
    { label: 'My Profile',      page: 'profile',        icon: <IconUser />     },
  ],
  owner: [
    { section: 'Main' },
    { label: 'Dashboard',             page: 'dashboard', icon: <IconGrid />     },
  ...
  customer: [
    ...
    { section: 'Shop' },
    { label: 'Register My Shop',  page: 'shop',          icon: <IconHome />  },
    { section: 'Account' },
    { label: 'My Profile',        page: 'profile',       icon: <IconUser />  },
  ],
};
...
function LogoIcon() {
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <rect x="11" y="4" width="22" height="36" rx="4" ry="4"
        stroke="currentColor" strokeWidth="2" fill="none" />
      <rect x="11" y="4" width="22" height="36" rx="4" ry="4"
        fill="currentColor" opacity="0.10" />
      <rect x="14" y="9" width="16" height="22" rx="2"
        fill="currentColor" opacity="0.15" />
      <rect x="19" y="6.5" width="6" height="1.5" rx="0.75"
        fill="currentColor" opacity="0.5" />
      <rect x="20" y="34" width="4" height="2" rx="1"
        fill="currentColor" opacity="0.6" />
      <circle cx="16" cy="11" r="1.2" fill="currentColor" opacity="0.8" />
      <polyline points="19,17 19,20 24,20 24,23 27,23"
...
/* ═══════════════════════════════════════
   SIDEBAR COMPONENT
   ─ collapsed / onCollapse lifted to DashboardLayout
   so the main content area can mirror the sidebar width
═══════════════════════════════════════ */
export default function Sidebar({
  ...
  activeSection,
  isOpen,
  collapsed,        // ← now a prop (owned by DashboardLayout)
  onCollapse,       // ← toggle callback from DashboardLayout
}) {
