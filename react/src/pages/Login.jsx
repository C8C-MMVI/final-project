...
function ForgotPasswordModal({ open, onClose, email, setEmail, emailError, loading, sent, onSubmit }) {
  return (
    <div className={`login-modal-overlay${open ? ' show' : ''}`} onClick={onClose}>
      <div className="login-modal-card" onClick={e => e.stopPropagation()}>
        ...
        {!sent ? (
          <>
            ...
            <form onSubmit={onSubmit} noValidate className="login-form">
              <div className="login-field">
                ...
                {emailError && <span className="login-field-error show">⚠ {emailError}</span>}
              </div>
              <button type="submit" disabled={loading} className={`login-btn-signin${loading ? ' loading' : ''}`}>
          ...
          <div className="login-modal-success">
            ...
            <button type="button" onClick={onClose} className="login-modal-back-btn">
              Back to login
            </button>
          </div>
        )}
...
export default function Login({ onLogin }) {
  const {
    ...
    forgotLoading, forgotEmailError, forgotSent,
    handleForgotSubmit,
    googleLogin,
  } = useLogin({ onLogin });
  ...
  return (
    <>
      <div className="login-page">
        <div className="login-container">
          ...
          <div className="login-right">
            ...
            <div className="login-welcome-sub">Sign in to continue to TechnoLogs.</div>
      ...
      <ForgotPasswordModal
        open={forgotOpen} onClose={closeForgot}
        email={forgotEmail} setEmail={setForgotEmail}
        emailError={forgotEmailError}
        loading={forgotLoading} sent={forgotSent}
